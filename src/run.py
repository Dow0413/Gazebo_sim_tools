...
def main() -> int:
    ...
    stages = {
        "glb_to_world": [root / "GS_GZ/glb_to_gazebo.py"],
        "to_pcd": [root / "GZ_PCD/to_pcd.py"],
        "all": [root / "GS_GZ/glb_to_gazebo.py", root / "GZ_PCD/to_pcd.py"],
    }
    if mode not in stages:
        raise SystemExit("mode must be glb_to_world, to_pcd, or all")
